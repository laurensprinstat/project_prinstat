...
```
###################################################
# 4  Power calc with bonfer correction          #
###################################################

```{R}
#dbp_end and dbp_dif are simulated simultaneously, but dependent on each other with the original means. 
#Both are tested for a significant effect and the lowest p-value of the two is taken, but corrected via the Bonferroni method.
data1 <- data[data$treatment == "Treatment 1",]
data2 <- data[data$treatment == "Placebo",]
...
for(i in 1 : 10000) {
  ...
  sim_end2 <- rnorm(n2, mean_end2, sd_end2) #the means in both groups are equal to simulate H0
  sim_base1 <- rnorm(n1, mean_dif1, sd_dif1)
  sim_base2 <- rnorm(n2, mean_dif1, sd_dif1)
  sim_dif1 <- sim_end1 - sim_base1 #sim_dif is calculated as a function of sim_end to simulate dependence
  sim_dif2 <- sim_end2 - sim_base2
  t_test_end <- t.test(sim_end1, sim_end2, alternative = "less", mu = 0, paired = F, var.equal = F)
  t_test_dif <- t.test(sim_dif1, sim_dif2, alternative = "less", mu = 0, paired = F, var.equal = F)
