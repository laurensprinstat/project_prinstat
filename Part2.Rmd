...
for(i in 1:n_sim){
  ...
  tt_end[i] <- t.test(dbp_end ~ treatment, data=pdata, alternative="less", var.equal=FALSE)$statistic
  tt_dif[i] <- t.test(dbpdif ~ treatment, data=pdata, alternative="less", var.equal=FALSE)$statistic
}
...
while (min(c(p_end, p_dif)) > 0.05) {
  ...
  tt_obs_end <- t.test(adj_t_end, r_p_end, alternative="less", var.equal=FALSE)$statistic
  tt_obs_dif <- t.test(adj_t_dif, r_p_dif, alternative="less", var.equal=FALSE)$statistic
  # P-value
...
[1] "Smallest detected difference: 0.86"
[1] "P-value final measure: 0.06463"
[1] "P-value difference dbp5-3: 0.04967"
...
for(i in 1:n_sim){
  r_placebo <- rnorm(n=n_placebo, mean=mean_p_dif, sd=sd_p_dif)
  r_treat <- rnorm(n=n_treatment, mean=mean_t_dif + dif_dif - adj, sd=sd_t_dif)
  r_placebo_0 <- rnorm(n=n_placebo, mean=0, sd=sd_placebo)
  r_treat_0 <- rnorm(n=n_treatment, mean=0, sd=sd_treatment)
  # One sided pooled t-test
  test <- t.test(r_treat, r_placebo, alternative="less", var.equal=FALSE)
