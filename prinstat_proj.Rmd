...
# By treatment group
corrplot.mixed(cor(data_raw[data_raw$treatment == 1,] %>% select(sex, weight, age, dose, dbp_mean, dbpdif, dbp_end)), lower="number", upper="ellipse", tl.pos="d")
corrplot.mixed(cor(data_raw[data_raw$treatment == 2,] %>% select(sex, weight, age, dose, dbp_mean, dbpdif, dbp_end)), lower="number", upper="ellipse", tl.pos="d")
corrplot.mixed(cor(data_raw[data_raw$treatment == 3,] %>% select(sex, weight, age, dose, dbp_mean, dbpdif, dbp_end)), lower="number", upper="ellipse", tl.pos="d")
```
...
plot(treat3$weight, treat3$dose)
...
(lvp) the previous time 0.6 was (intended) to be applied to limit the bias applied to the correlation tests between dosage and other factors.
...
(lvp) The overall mean across the 3 measurements would be 89.33 while the selection criteria applied seems to be 90+. Which adds to the belief that it’s an error. Normally I'd be ok with keeping it and at first I wanted to do that too, but not a single other patient has an average below 90. Thus it seems very odd from different perspectives.
...
table_dbp_end <- with(data, tapply(dbp_end, treatment, function(x) 
  paste0(round(mean(x)), " (", IQR(x), ")")))
table_dbpdif <- with(data, tapply(dbpdif, treatment, function(x) paste(round(mean(x)), "±", round(sd(x)))))
table_all <- noquote ( t( rbind(table_dbp3, table_dbp_end, table_dbpdif)))
rownames(table_all) <- c("Traetment 1", "Placebo", "Treatment 3")
colnames(table_all) <- c("dbp3 - median (IQR)", "dbp5 - median (IQR)", "dpbdif - mean ± sd")
table_all
```
...
table_total <- with(data, tapply(dbpdif, treatment, function(x) paste(round(mean(x)), "±", round(sd(x)))))
table_male <- with(data[which(data$sex == 1),], tapply(dbpdif, treatment, function(x) 
  paste(round(mean(x)), "±", round(sd(x)))))
