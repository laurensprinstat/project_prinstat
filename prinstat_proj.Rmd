...
one <- nrow(original[original$trt==1,]) - nrow(data[data$treatment=="Treatment 1",])
two <- nrow(original[original$trt==2,]) - nrow(data[data$treatment=="Placebo",])
three <- nrow(original[original$trt==3,]) - nrow(data[data$treatment=="Treatment 3",])
one
two
three
...
```{r}
# Check normality
qqnorm(treat1$dbpdif, main="DBP difference treatment group 1");qqline(treat1$dbpdif)
qqnorm(placebo$dbpdif, main="DBP difference placebo");qqline(placebo$dbpdif)
qqnorm(treat3$dbpdif, main="DBP difference treatment group 3");qqline(treat3$dbpdif)

qqnorm(treat1$dbp_end, main="Final DBP treatment group 1");qqline(treat1$dbp_end)
qqnorm(placebo$dbp_end, main="Final DBP placebo");qqline(placebo$dbp_end)
qqnorm(treat3$dbp_end, main="Final DBP treatment group 3");qqline(treat3$dbp_end)
...
###############################
# 7a. Power of the gender difference
###############################
```{R}
...
mu2f <- mean(data23$dbpdif[data23$treatment == "2" & data23$sex == "female"])
mu3f <- mean(data23$dbpdif[data23$treatment == "3" & data23$sex == "female"])
mu2m <- mean(data23$dbpdif[data23$treatment == "2" & data23$sex == "male"])
mu3m <- mean(data23$dbpdif[data23$treatment == "3" & data23$sex == "male"])

# Finding the sd of each gender-treatment combination in the real dataset
sd2f <- sd(data23$dbpdif[data23$treatment == "2" & data23$sex == "female"])
sd3f <- sd(data23$dbpdif[data23$treatment == "3" & data23$sex == "female"])
sd2m <- sd(data23$dbpdif[data23$treatment == "2" & data23$sex == "male"])
sd3m <- sd(data23$dbpdif[data23$treatment == "3" & data23$sex == "male"])

# Simulation study to find the power of the ANOVA-interaction test
power_data <- vector(,1000)
set.seed(2018)
for(i in 1:1000) {
  sim2f <- rnorm(19, mean = mu2f, sd = sd2f)
  sim3f <- rnorm(20, mean = mu3f - 2, sd = sd3f) # mean decreased by 2 for females
  sim2m <- rnorm(22, mean = mu2m, sd = sd2m)
  sim3m <- rnorm(19, mean = mu3m + 2, sd = sd3m) # mean increased by 2 for males
  # Males and females now differ 4 mmHg in treatment effect on average
  
  #Creating a dataframe from the simulated values
  dbpdif <- c(sim2f, sim3f, sim2m, sim3m)
  genders <- c(rep("female", 39), rep("male", 41))
  treatments <- c(rep("placebo", 19), rep("treatment3", 20), rep("placebo", 22), rep("treatment3", 19))
