...
```
We can reject the 0 hypotheses of the proprtions of being under the cut-off being equal at the 95% confidence level.
...
Since the cutoff defined as (dbp5-dbp3)<10mmHg seemed rather wide to us, we decided to also check if (dbp5-dbp3)< -10mmHg,thereby looking for if the Treatment group beats the placebo group with at least 10mmHg.
```{r}
data23$binaryrev <- 0
...
```
Here we also see a rejection of the H0, but much stronger. A z-value of 3.52 relates to a confidence level of 99.98%, whereas in the first case a 1.76 Z-score relates to a 96.08% confidence level.
