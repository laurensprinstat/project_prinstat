...
```{r}
#basic descriptives function
descriptives <- function(variable){
  variable <- as.numeric(variable)
print(paste('mean is',mean(variable)))
print(paste('median is',median(variable)))
print(paste('sd is',sd(variable)))
print(paste('min is',min(variable)))
print(paste('max is',max(variable)))
print(summary(variable~data$treatment))
}
#Research question: difference between treatment groups
#look at treatment groups

#final difference
#control around 0, other dropped by 10 median
descriptives(data$dbpdif)

#at start1, we can see at visit 1, little to no difference 
#upper range for control is somewhat larger  
descriptives(data$dbp1)

#at start2
descriptives(data$dbp2)

#at start3, same trend visit 3
descriptives(data$dbp3)

#at end, same trend visit 5
descriptives(data$dbp5)
...
```
Conclusion q2: At first glance we can see that the groups had a similar bp at intake visits, but the treatment groups(1,3) had a lower bp than before at the final visit. The control group(2) remained more or less stable throughout the study. We can see this as a first indication that the treatment groups did achieve a lower bp.
